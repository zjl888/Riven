...
public class AliasToBeanResultTransformerPlus extends AliasToBeanResultTransformer {
    ...
    public <T> AliasToBeanResultTransformerPlus(Class<T> resultClass) {
        super(resultClass);
        if (resultClass == null) {
    ...
    public Object transformTuple(Object tuple[], String aliases[]) {
        Object result;
        try {
            if (!isInitialized) initialize(aliases);
            else check(aliases);
            result = resultClass.newInstance();
            for (int i = 0; i < aliases.length; i++)
                if (setters[i] != null) setters[i].set(result, tuple[i], null);
    ...
    public boolean equals(Object o) {
        ...
        AliasToBeanResultTransformerPlus that = (AliasToBeanResultTransformerPlus) o;
